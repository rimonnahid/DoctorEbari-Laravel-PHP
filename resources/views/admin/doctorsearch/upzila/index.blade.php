...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-12">
        <h3 class="font-weight-bold mt-3">Upzila List <a href="{{ route('create.upzila') }}" class="btn btn-sm btn-primary float-right">Add Upzila</a></h3>
    </div>
</div>
		...
		<div class="table-responsive">
            <table class="table table-striped table-bordered table-hover dataTables-example" >
                ...
                @foreach($upzilas as $upzila)
                    <tr class="gradeX">
                        <td>{{ $count++ }}</td>
                        <td>{{ $upzila->upzila_name }}</td>
                        <td>{{ $upzila->district->district_name }}</td>
                        <td>{{ $upzila->division_id }}</td>
                        <td>{{ $upzila->created_at->diffForHumans() }}</td>
                        <td class="center">
                        	<a href="{{ route('edit.upzila',$upzila->id) }}" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i></a>
                        	<a href="{{ route('delete.upzila',$upzila->id) }}" class="btn btn-danger btn-sm" id="delete"><i class="fa fa-trash-o"></i></a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
