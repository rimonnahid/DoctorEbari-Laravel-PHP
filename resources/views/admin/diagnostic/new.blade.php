...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-12">
        <h3 class="font-weight-bold mt-3">New Diagnostic Form <a href="{{ route('list.diagnostic') }}" class="btn btn-sm btn-primary float-right">All Diagnostics</a></h3>
    </div>
</div>
<div class="wrapper wrapper-content">
    <div class="ibox-content">
        <form action="{{ route('store.diagnostic') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="">Diagnostic Name</label>
                        <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}" placeholder="enter diagnostic name" required>
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('slug') is-invalid @enderror" name="slug" value="{{ old('slug') }}" placeholder="ex: diagnostic-name" required>
                        @error('slug')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                ...
                <div class="col-md-12 form-group">
                    ...
                    @enderror
                    <button type="submit" class="btn btn-sm btn-block btn-success mt-3">New diagnostic</button>
                </div>
            </div>
        </form>
    </div>
